...
extern "C" {
    ...
    IP_API_Reply IP_addUnitConfig(IP_UUTHandle ahUUTHandle,struct IP_kvStruct  *kvStruct[],int size);
    ///@param has to be the following format(struct key, val) key ->Key7 and val->Val7, key->Key2 val->Val2, key->rootKey val->rootVal"
    
    IP_API_Reply IP_addTestBOM(IP_UUTHandle ahUUTHandle,struct IP_kvStruct  *kvStruct[],int size);
